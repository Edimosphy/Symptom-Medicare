...
if not st.session_state.name_entered:
    with st.form("name_form"):
        ...
        name_submit = st.form_submit_button("Submit Name")
        if name_submit and user_name.strip():
            st.session_state.name = user_name.strip()
            st.session_state.name_entered = True
            st.experimental_rerun()  # Rerun to show next step

# --- Step 2: Welcome and Show Symptom Form ---
if st.session_state.name_entered:
    st.success(f"Welcome, {st.session_state.name}! Please proceed to select your symptoms below:")
    ...
    if submitted:
        ...
        st.success(f"🎯 {st.session_state.name}, based on your symptoms, the most likely disease is: **{prediction}**")
